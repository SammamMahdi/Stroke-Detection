...
# Real-time stroke detection using video input
def detect_stroke_from_video():
    cap = cv2.VideoCapture(0)  # Use webcam. Replace with video file path if needed
    ...
    while cap.isOpened():
        ...
        #rotate the frame
        features = get_landmarks(frame_rgb)
        ...
        if features and len(features) == 115:  # Ensure feature length matches what the model expects
            ...
            cv2.putText(frame, label, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        else:
            cv2.putText(frame, "Face not detected", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA )
